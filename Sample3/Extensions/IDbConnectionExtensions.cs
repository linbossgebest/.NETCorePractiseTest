...
namespace Sample03.Extensions
{
    ...
    public static class IDbConnectionExtensions
    {
        ...
        public static List<DbTable> GetCurrentDatabaseTableList(this IDbConnection dbConnection, DatabaseType dbType)
        {
            ...
            {
                table.Columns = dbConnection.GetColumnsByTableName(dbType,table.TableName);
                table.Columns.ForEach(column => 
                {
                    var csharpType = DbColumnTypeCollection.DbColumnDataTypes.FirstOrDefault(f =>f.DatabaseType==dbType&&f.ColumnTypes.Split(',').Any(k=>k.Trim().Equals(column.ColumnType,StringComparison.OrdinalIgnoreCase)))?.CSharpType;
                    if (csharpType.IsNullOrWhiteSpace())
                    {
                        throw new SqlTypeException($"未从字典中找到\"{column.ColumnType}\"对应的C#数据类型，请更新DbColumnTypeCollection类型映射字典。");
                    }
